const Store = require('./abacadisease-store');
...
const moduleName = 'Abaca Disease Management Project';
const userId = 1;

class AbacaDiseaseManagementService {
  constructor(store) {
  }
  // Add
  async add(req, res, next) {
    try {
      ...
      for (let i = 0; i < jsonData.length; i++) {
        ...
        if (row['Date Eradicated'] && typeof row['Date Eradicated'] === 'number') {
          row['Date Eradicated'] = convertExcelDate(row['Date Eradicated']);
        }
...
module.exports = AbacaDiseaseManagementService;