const Store = require("./materials-store");
    ...
    try {
      ...
      for (let i = 0; i < jsonData.length; i++) {
        ...
        if (
          ...
          row["Date Distributed"] = convertExcelDate(
            row["Date Distributed"]
          );
        }
      ...
      for (const row of rowsToAdd) {
        ...
        await logs.add({
          ...
          module: moduleName,
          data: row,
          action: "imported a new row in IEC Materials table",
          ...body
        });
        rowsAdded.push(row);
      }
  ...
  async delete(req, res, next) {
    try {
      ...
      logs.add({
        uuid: userId,
        module: moduleName,
        action: "deleted a row in IEC Materials table",
        ...body
      });
