const Store = require("./pmsurvived-store");
    ...
    try {
      ...
      for (let i = 0; i < jsonData.length; i++) {
        ...
        if (
          ...
          !row["Municipality"] ||
          !row["Barangay"] ||
          !row["Number of PM Available During Establishment"] ||
          !row["Variety"] ||
          !row["Date Received"] ||
          !row["Number of PM Planted"] ||
          !row["Number of PM Survived"]
        ) {
        ...
        if (
          row["Date Received"] &&
          typeof row["Date Received"] === "number"
        ) {
          row["Date Received"] = convertExcelDate(
            row["Date Received"]
          );
        }
    ...
    async add1(req, res, next) {
        try {
            ...
            for (let i = 0; i < jsonData.length; i++) {
                ...
                if (
                    row["Date Recieved"] &&
                    typeof row["Date Recieved"] === "number"
                ) {
                    row["Date Recieved"] = convertExcelDate(
                        row["Date Recieved"]
                    );
                }
...
module.exports = PmSurvivedService;
